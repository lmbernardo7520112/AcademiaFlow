...
export const turmasRoutes: FastifyPluginAsyncZod = async (fastify: FastifyInstance) => {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const turma = await turmasService.create(payload);
  ...
  fastify.get('/', async (_request: FastifyRequest, reply: FastifyReply) => {
    try {
      const turmas = await turmasService.list();
  ...
  fastify.get('/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const turma = await turmasService.getById(id);
  ...
  fastify.put(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const turma = await turmasService.update(id, payload);
        reply.send({ success: true, data: turma });
      } catch (error: Error | unknown) {
        reply.code(400).send({
          success: false,
  ...
  fastify.delete('/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { id } = request.params as { id: string };
      const result = await turmasService.softDelete(id);
      reply.send(result);
    } catch (error: Error | unknown) {
      reply.code(400).send({
        success: false,
